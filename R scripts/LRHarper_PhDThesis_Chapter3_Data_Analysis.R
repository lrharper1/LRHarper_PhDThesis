#' ---
#' Title: "Data analysis for Chapter 3 of PhD Thesis by Dr. Lynsey R Harper'
...
## Change Anas carolinensis to Anas spp.
## Change Sus scrofa to Sus scrofa domesticus
## Change Canis lupus to Canis lupus familaris
colnames(vert.dat) <- gsub("Anas_carolinensis", "Anas_spp.", colnames(vert.dat))
colnames(vert.dat) <- gsub("Sus_scrofa", "Sus_scrofa_domesticus", colnames(vert.dat))
colnames(vert.dat) <- gsub("Canis_lupus", "Canis_lupus_familiaris", colnames(vert.dat))
...
## Make sure all columns are classed as numeric
vert_binary[,2:61] <- sapply(vert_binary[,2:61], as.numeric)
...
p1m <- p1m + geom_boxplot(aes(x=factor(Amphibian), y=fit), outlier.shape = NA, alpha=0.7)
p1m <- p1m + labs(title="", y=expression(paste("Probability of ", italic("T. cristatus"))), x = "Number of other amphibian species") 
...
p2m <- p2m + geom_boxplot(aes(x=factor(Fish), y=fit), outlier.shape = NA, alpha=0.7)
p2m <- p2m + labs(title="", y=expression(paste("Probability of ", italic("T. cristatus"))), x = "Number of fish species") 
...
p3m <- p3m + geom_boxplot(aes(x=factor(Waterfowl), y=fit), outlier.shape = NA, alpha=0.7)
p3m <- p3m + labs(title="", y=expression(paste("Probability of ", italic("T. cristatus"))), x = "Number of waterfowl species") 
...
p4m <- p4m + geom_boxplot(aes(x=factor(Woodland.Bird), y=fit), outlier.shape = NA, alpha=0.7)
p4m <- p4m + labs(title="", y=expression(paste("Probability of ", italic("T. cristatus"))), x = "Number of terrestrial bird species") 
...
p5m <- p5m + geom_boxplot(aes(x=factor(Mammal), y=fit), outlier.shape = NA, alpha=0.7)
p5m <- p5m + labs(title="", y=expression(paste("Probability of ", italic("T. cristatus"))), x = "Number of mammal species") 
...
p7 <- p7 + scale_fill_manual(name=expression(italic("T. cristatus")),
                             values=c("grey45","orange"),
                             breaks=c("P","N"),
                             labels=c("Positive",
                                      "Negative"))
...
p8 <- p8 + scale_fill_manual(name=expression(italic("T. cristatus")),
                             values=c("grey45","orange"),
                             breaks=c("P","N"),
                             labels=c("Positive",
                                      "Negative"))
...
p12 <- p12 + labs(x = expression(italic("L. vulgaris")),
...
p13 <- p13 + scale_x_discrete(breaks=c("0", "1"),
                              labels=c("Absent", "Present"))
p13 <- p13 + labs(x = expression(italic("B. bufo")),y="") 
...
p14 <- p14 + scale_x_discrete(breaks=c("0", "1"),
                              labels=c("Absent", "Present"))
p14 <- p14 + labs(x = expression(italic("G. aculeatus")), 
                  y = "") 
...
p15 <- p15 + scale_x_discrete(breaks=c("0", "1"),
                              labels=c("Absent", "Present"))
p15 <- p15 + labs(x = expression(italic("S. carolinensis")), 
                  y = expression(paste("Probability of ", italic("T. cristatus"))))
